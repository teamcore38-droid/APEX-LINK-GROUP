      ...
      <div className="container mx-auto max-w-6xl px-3 sm:px-4">
        ...
        <div className="mt-4 grid gap-5 xl:grid-cols-[0.85fr_1.15fr]">
          <aside className="space-y-4">
            ...
            <div className="grid gap-3 sm:gap-3.5">
              {contactCards.map(({ icon: Icon, title, body }) => (
                <article
                  key={title}
                  className="flex items-start gap-3.5 rounded-[20px] bg-white p-3.5 shadow-[0_18px_40px_rgba(53, 26, 17,0.08)] sm:p-4"
                >
                  <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-brand-light text-brand-primary">
                    <Icon size={18} />
                  </div>
                  <div>
                    <h3 className="font-serif text-lg font-bold text-brand-dark leading-tight">{title}</h3>
                    <div className="mt-1 whitespace-pre-line text-xs leading-relaxed text-gray-600">{body}</div>
                  </div>
                </article>
              ))}
            </div>
            ...
            <div className="hidden xl:flex flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-[#bbf7d0] bg-[#f0fdf4] p-4 sm:p-5">
              <div className="flex items-center gap-3.5">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#00d757] text-white shadow-sm">
                ...
                <div>
                  <h3 className="font-serif text-base font-bold text-brand-dark">Prefer to chat with us?</h3>
                  <p className="text-xs text-gray-600">Contact our Customer Care team on WhatsApp for quick assistance.</p>
                </div>
              </div>
              ...
              <a
                ...
                className="inline-flex w-full sm:w-auto items-center justify-center gap-2 rounded-xl bg-[#00d757] hover:bg-[#00c04d] px-4 py-2.5 text-xs font-bold uppercase tracking-wider text-white shadow-sm transition-all duration-200 hover:shadow-md shrink-0"
              >
          ...
          <section className="rounded-[28px] bg-white p-6 shadow-[0_18px_40px_rgba(53, 26, 17,0.08)] sm:p-8">
            ...
            {/* Mobile WhatsApp Box */}
            <div className="mt-6 flex xl:hidden flex-col sm:flex-row items-center justify-between gap-4 rounded-2xl border border-[#bbf7d0] bg-[#f0fdf4] p-4 sm:p-5">
              <div className="flex items-center gap-3.5">
                <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full bg-[#00d757] text-white shadow-sm">
                  <svg className="h-5 w-5 fill-current" viewBox="0 0 24 24">
                    <path d="M.057 24l1.687-6.163c-1.041-1.804-1.588-3.849-1.587-5.946.003-6.556 5.338-11.891 11.893-11.891 3.181.001 6.167 1.24 8.413 3.488 2.245 2.248 3.481 5.236 3.48 8.414-.003 6.557-5.338 11.892-11.893 11.892-1.99-.001-3.951-.5-5.688-1.448l-6.305 1.654zm6.597-3.807c1.676.995 3.276 1.591 5.392 1.592 5.448 0 9.886-4.434 9.889-9.885.002-5.462-4.415-9.89-9.881-9.892-5.452 0-9.887 4.434-9.889 9.884-.001 2.225.651 3.891 1.746 5.634l-.999 3.648 3.742-.981zm11.387-5.464c-.074-.124-.272-.198-.57-.347-.297-.149-1.758-.868-2.031-.967-.272-.099-.47-.149-.669.149-.198.297-.768.967-.941 1.165-.173.198-.347.223-.644.074-.297-.149-1.255-.462-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.297-.347.446-.521.151-.172.2-.296.3-.495.099-.198.05-.372-.025-.521-.075-.148-.669-1.611-.916-2.206-.242-.579-.487-.501-.669-.51l-.57-.01c-.198 0-.52.074-.792.372s-1.04 1.016-1.04 2.479 1.065 2.876 1.213 3.074c.149.198 2.095 3.2 5.076 4.487.709.306 1.263.489 1.694.626.712.226 1.36.194 1.872.118.571-.085 1.758-.719 2.006-1.413.248-.695.248-1.29.173-1.414z"/>
