                  ...
                  {cartItems.map(item => (
                    <li key={`${item.product}-${item.variantId || 'default'}-${item.size || 'nosize'}-${item.color || 'nocolor'}`} className="p-6 flex flex-col md:grid md:grid-cols-5 items-center gap-4">
                        ...
                        <Link to={buildProductPath({ _id: item.product, slug: item.slug, name: item.name })} className="ml-4 font-semibold text-brand-dark hover:text-brand-primary">
                          ...
                          {(item.variantLabel || item.size || item.color) && (
                            <span className="mt-1 flex flex-wrap items-center gap-1.5 text-xs font-medium text-gray-500">
                              ...
                              {item.color && (
                                <span className="rounded-md bg-[#efebe6] px-2 py-0.5 text-[11px] font-bold text-[#4a3b32]">
                                  Color: {item.color}
                                </span>
                              )}
                            </span>
                      ...
                      <div className="flex justify-center w-full md:w-auto">
                        <div className="flex items-center rounded-xl border border-gray-200 bg-[#fff7ee] p-1 w-[110px] justify-between shadow-sm">
                          <button
                            type="button"
                            onClick={() => addToCart(item, Math.max(1, item.qty - 1))}
                            disabled={item.qty <= 1}
                            className="flex h-7 w-7 items-center justify-center rounded-lg text-brand-dark hover:bg-brand-light/70 transition disabled:opacity-30 disabled:hover:bg-transparent"
                            aria-label={`Decrease quantity for ${item.name}`}
                          >
                            <Minus size={14} />
                          </button>
                          <span className="font-semibold text-brand-dark text-sm w-6 text-center select-none">
                            {item.qty}
                          </span>
                          <button
                            type="button"
